...
class EIOPAProcessor:
    ...
    def process(self) -> Optional[Dict]:
        ...
        En plus des taux cibles (TARGET_MATURITIES), génère deux fichiers CSV
        contenant la courbe complète (maturités 0–150) :
          - RFR_[DATE]_NO_VA.csv
          - RFR_[DATE]_WITH_VA.csv

        Returns:
            Dictionnaire contenant les données extraites, avec les clés
            'rfr_no_va_csv' et 'rfr_with_va_csv' pointant vers les CSV créés.
        """
        ...
        # Lire l'onglet des taux spot (pour les taux cibles uniquement)
        df = self.read_excel_robust(excel_path, EXCEL_SHEET_RFR)
        if df is None:
            logger.error(f"Impossible de lire l'onglet '{EXCEL_SHEET_RFR}'")
            return None

        # Extraire les taux pour le pays cible (maturités cibles, format analyzer)
        ...
        # --- Export CSV RFR complets (NO_VA et WITH_VA, maturités 0–150) ---
        ...
        if no_va_path:
            logger.info(f"CSV NO_VA généré  : {no_va_path.name}")
        if with_va_path:
            logger.info(f"CSV WITH_VA généré: {with_va_path.name}")
...
def main():
    ...
    if result:
        ...
        if result['va']:
            print(f"\nVA: {result['va']*100:.2f}%")
        print(f"\nCSV NO_VA  : {result.get('rfr_no_va_csv')}")
        print(f"CSV WITH_VA: {result.get('rfr_with_va_csv')}")
    else:
